...
export function getAndInstruction(params) {
    ...
    if (isRegister(params[1])) {
        ...
        // Opcja 1: and rax, imm32 – specjalne kodowanie
        if (dst === 'rax') {
            // and rax, imm32: 48 25 xx xx xx xx
            let rex_str = '48';
            let opcode = '25';
            let imm_le = LE(params[1]);
            return rex_str + ' ' + opcode + ' ' + imm_le;
        }
        // Opcja 2: and reg, imm32: 48 81 E0+reg imm32
        let rex = 0x48;
        if (dst_id >= 8) rex = 0x49;
        let rex_str = rex.toString(16).toUpperCase();
        let opcode = '81';
        let modrm = (0xC0 | (0x4 << 3) | (dst_id & 7)).toString(16).toUpperCase(); // reg/opcode=100 (and), r/m=dst
        let imm_le = LE(params[1]);
...
// Przykłady użycia:
console.log(getAndInstruction(['rax', 'rax']));       // '48 21 C0'
console.log(getAndInstruction(['r12', 'r12']));       // '4D 21 E4'
console.log(getAndInstruction(['r9', '0x12345678'])); // '49 81 E1 78 56 34 12'