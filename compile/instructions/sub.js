...
function LE(hexstr) {
    ...
    // sub używa tylko 4 bajtów (imm32)
    return bytes.slice(0, 4).join(' ');
}

export function getSubInstruction(params) {
    ...
    if (isRegister(params[1])) {
        ...
        // Opcja 1: sub rax, imm32 – specjalne kodowanie
        if (dst === 'rax') {
        ...
        // Opcja 2: sub reg, imm32: 48 81 E8+reg imm32
        let rex = 0x48;
        if (dst_id >= 8) rex = 0x49;
        let rex_str = rex.toString(16).toUpperCase();
        let opcode = '81';
        let modrm = (0xC0 | (0x5 << 3) | (dst_id & 7)).toString(16).toUpperCase(); // reg/opcode=101 (sub), r/m=dst
        let imm_le = LE(params[1]);
...
// Przykłady użycia
console.log(getSubInstruction(['rax', 'rax']));       // '48 29 C0'
console.log(getSubInstruction(['r12', 'r12']));       // '4D 29 E4'
console.log(getSubInstruction(['r9', '0x12345678'])); // '49 81 ED 78 56 34 12'